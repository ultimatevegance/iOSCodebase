//
//  MSScrollingNavigationBar.swift
//  iOSAppTemplateSwift
//
...
/**
 ...
 Make sure to use a subclass of MSScrollingNavigationController for your UINavigationController;
 then to
...
@objc public protocol ScrollingNavigationControllerDelegate: NSObjectProtocol {
    ...
    ///   - state: the new state
    @objc optional func scrollingNavigationController(_ controller: MSScrollingNavigationController, didChangeState state: NavigationBarState)
    ...
    ///   - controller: the ScrollingNavigationController
    ///   - state: the new state
    @objc optional func scrollingNavigationController(_ controller: MSScrollingNavigationController, willChangeState state: NavigationBarState)
}
 ...
 */
@objcMembers
open class MSScrollingNavigationController: UINavigationController, UIGestureRecognizerDelegate {
    ...
    open func followScrollView(_ scrollableView: UIView, delay: Double = 0, scrollSpeedFactor: Double = 1, collapseDirection: NavigationBarCollapseDirection = .scrollDown, followers: [UIView] = []) {
        ...
        gestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(MSScrollingNavigationController.handlePan(_:)))
        gestureRecognizer?.maximumNumberOfTouches = 1
        gestureRecognizer?.delegate = self
        scrollableView.addGestureRecognizer(gestureRecognizer!)
        
        NotificationCenter.default.addObserver(self, selector: #selector(MSScrollingNavigationController.willResignActive(_:)), name: NSNotification.Name.UIApplicationWillResignActive, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MSScrollingNavigationController.didBecomeActive(_:)), name: NSNotification.Name.UIApplicationDidBecomeActive, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MSScrollingNavigationController.didRotate(_:)), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
